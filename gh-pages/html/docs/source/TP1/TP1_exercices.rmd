...
2. Calculer un vecteur `vec` de 100000 points correspondant à la loi normale de moyenne `m` et d'éacrt type `sd` correspondant à la moyenne, et à l'écart type, des durées d'éruption
3. Construire un histogramme de la durée d’éruption. Représenter l’histogramme en terme de densité plutôt qu’en termes d’effectifs (axe Y). Ajouter un titre, nommer les axes et colorer les barres de l’histogramme en vert et les traits de l’histogramme en rouge. Augmenter la taille du pas de l’histogramme à 20.
4. Ajouter la densité du vecteur de point `vec` à l'histogramme (*Indice : `lines()`, `density()`*. Que remarquez-vous ?
5. Utiliser la fonction `layout` pour afficher un scatter plot
...
xhist = hist(faithful$eruptions, plot=FALSE, breaks=lhist)
yhist = hist(faithful$waiting  , plot=FALSE, breaks=lhist)
barplot(xhist$density, axes=FALSE, ylim=c(0, max(xhist$density, xy)),space=0) 
barplot(yhist$density, axes=FALSE, xlim=c(0, max(yhist$density, yy)),space=0, horiz=TRUE)


# # evaluation points for the overlaid density
# xx <- seq(min(faithful$eruptions), max(faithful$eruptions), length.out=num.dnorm)
# yx <- seq(min(faithful$waiting)  , max(faithful$waiting)  , length.out=num.dnorm)
# 
# # density points
# xy <- dnorm(xx, mean=mean(faithful$eruptions), sd=sd(faithful$eruptions))
# yy <- dnorm(yx, mean=mean(faithful$waiting)  , sd=sd(faithful$waiting))
# 
# par(mar=c(0, pext, 0, 0))
# 
# # barplot pour "eruption"
# barplot(xhist$density, axes=FALSE, ylim=c(0, max(xhist$density, xy)),space=0) 
# # line pour "eruption"
# lines(seq(from=0, to=lhist-1, length.out=num.dnorm), xy, col="blue") 
# 
# # barplot pour "waiting"
# barplot(yhist$density, axes=FALSE, xlim=c(0, max(yhist$density, yy)),space=0, horiz=TRUE)
# # line pour "waiting"
# lines(yy, seq(from=0, to=lhist-1, length.out=num.dnorm), col="blue")
```
...
1. creer une fonction `simul` qui prend comme argument un entier compris entre 1 et 6, et qui retourne la somme correspondante.
2. creer un vecteur `tirage`, simulant le resultat de 1000 tirages de dès.
