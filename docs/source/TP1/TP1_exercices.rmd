...
```{r exercice12, echo = FALSE, eval=FALSE}
...
```{r exercice14, echo = FALSE, eval = FALSE}
#1
data(iris)
str(iris)
moyenneET(2)
...
```{r exercice21.2, echo = FALSE, eval=FALSE}
#2
...
?
5. Afin de mieux apprehender la distribution des données, il est possible d'afficher plusieurs graphique sur une même grille. L'objectif de cette partie est d'afficher le scatterplot de la question 1, et au dessus et sur la droite, les histogramme des variables "eruption" et "waiting", respectivement. Pour cela, nous utiliserons la fonction `layout()`.

5.1 Creer la matrice suivante :
...
* premiere colonne : 70% de la largeur total
* deuxième colonne : 30% de la largeur total
* premiere ligne : 30% de la largeur total
* deuxième ligne : 70% de la largeur total
...
```{r exercice25, echo = FALSE, eval=FALSE}
library(cartography)
...
Cette exercice a pour objectif de reconstruire ce profil à partir du package "chromosomes" (développé par B. Job, Gustave Roussy) et du jeu de données A01.ASPCF, issu de l'analyse par des microarrays du type "Oncoscan" (Affymetrix) de la tumeur d'un patient.

![Profil CGH](img/A01.ASPCF.png)
...
1.1 Quelle est la classe de la variable `cs` ?
1.2 Combien le génome contient-il de chromosomes ? (*Indice : parcourir l'obet `cs` avec la fonction `str`*)
1.3 Quelle est la taille du génome humain (dans sa version hg19) ?

2. Charger le fichier `A01.ascat.ASPCF.RDS` dans la variable `cgh`
2.1 Quelle est la classe de la variable `cgh` ?
2.2 Combien d'éléments contient cette variable ?
2.3 Quelle est le genre du patient à qui correspond le profil ?

3. Construction du profil CNA
3.1 Construction des vecteurs de points à afficher depuis la variable `cgh`:
...
3.2 En utilisant la variable `cs`, construisez le vecteur `chr_length`, contenant les longueurs des chromosomes. Utiliser la fonction `names` et la variable `cs` pour nommer tous les éléments du vecteur `chr_length`.

3.3 Convertir les positions relatives au chromosome du vecteur `x_snp` en position absolue sur le génome (vecteur `x_snp_abs`)

3.4 Afin d'afficher une version lissée du signal mesuré, calculer la "running médiane" (*Indice : fonction `runmed`*) à partir du vecteur `l2r_tumor`

3.5 A partir de ces vecteurs, essayez de reproduire l'image suivante.
...
```{r exercice26, echo = FALSE, eval = FALSE}
#1
