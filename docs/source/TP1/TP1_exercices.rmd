...
<<<<<<< HEAD
## Exercice 13 : boucle while et for
...
for (j in 1:10){
  ...
  matrice[,j] = val
>>>>>>> 541854946e6d99d313a33523d617740bb92bcc5b
}
...
<<<<<<< HEAD
## Exercice 14 : Création de fonction
=======
## Exercice 14
>>>>>>> 541854946e6d99d313a33523d617740bb92bcc5b
...
<<<<<<< HEAD
# Exercice 15 : Création d’une fonction, traitement de chaînes de caractères
=======
...
```{r exercice15mail, echo = TRUE, eval = FALSE}
...
<<<<<<< HEAD
# Exercice 16 : Fonctions appliquées aux éléments d’une liste
=======
...
<<<<<<< HEAD
## Exercice 17 : Lire les données d’un fichier : fonction `read.table`
=======
...
<<<<<<< HEAD
## Exercice 18 : Enregistrer des données
=======
...
## Exercice 20 : quelques graphique des base en R 

1. Charger le jeu de données `iris`.
2. Sur la colonne contenant des données qualitatives, réaliser une représentation en camembert (pie chart), puis en bâtons (barplot).
3. Taper la ligne de commande suivante : 

```{r exercice20.1, echo=TRUE, eval=FALSE}
data(iris)
summary(iris)
```
...
4. Une représentation adéquate est la boîte à moustache (boxplot). Créer un boxplot pour les 4 variables numériques du jeu de données `iris`.
5. Réaliser le même graphique en ajoutant un titre et en supprimant les valeurs extrêmes.
6. Représenter le pie chart et le boxplot sur la même fenêtre graphique. *Indice `par()`, option mfrow*
7. Représenter un scatter plot simple avec en abscisse la longueur des pétales et en ordonnées leur largeur. *Indice `plot`*
8. Afin de découvrir les différents paramètre de la fonction `plot`, refaite le même graphique qu'à la fonction précédente en :    * augmentant la taille des points
   ...
   * (question plus difficiel) colorant les points en fonction de l'espèce : setosa = noir ; versicolor = rouge ; virginica = vert
   
Pour savoir comment se comporte les variables les unes par rapport aux autres, on pourrait créer autant de scatter plot que nécessaire mais il existe une fonction qui permet de faire ceci rapidement : `pairs`.

9. Utiliser la fonction `pairs` sur les données numérique du jeu de données `iris`. Colorer les points en fonction de l'espèce.
10. Exporter ce graphique en pdf sur votre machine. *Indice `pdf()` ; `dev.off()`*
...
## Exercice 21 : distribution et density plot

1. Charger le jeu de données `airquality`.
2. Prendre connaissance des donéess (dimension, type)
3. Construire une histogramme de la variable `Ozone`. Représenter l'histogramme en terme de fréquence plutôt qu'en terme d'effectif (axe Y). Ajouter un titre, nommer les axes, et colorer les barres de l'histogramme en gris  et les traits de l'histogramme en bleu. Augmenter la taille  du pas de l'histogramme à 30.
...
Pour aller plus loi, et faire des graphiques encore plus jolis, il existe un package, `ggplot2`, qui possède sa propre grammaire. Pour reproduire le plot précédent, rien de plus simple : 
