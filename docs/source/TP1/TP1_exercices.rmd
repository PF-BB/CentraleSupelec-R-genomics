...
4. Supprimer la matrice `A` de l'environnement grâce à la fonction `rm`
5. Charger le fichier `matriceA.Rdata`
6. Sauver la matrice `A` au format `.Rds` dans le fichier `matriceA.Rds` grâce à la fonction `saveRDS`.
7. Charger le fichier `matriceA.rds` avec la fonction `readRDS`.
8. Sauver toutes les variables dans un fichier nommés "données.Rdata"
