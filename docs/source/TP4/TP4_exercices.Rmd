...
```{r Q1.2 Profil_expression, eval = TRUE, echo = FALSE}
...
```{r Q2.1 Table de contingence, echo=TRUE, eval = TRUE}
...
```{r Q2.1 Fisher, echo = FALSE, eval = TRUE}
### Taille de la liste 
No = length(erbb)
...
```{r Q3.1 selection_gene, eval = TRUE, echo = FALSE}
updown = read.table("data_TP4/dataMAplot_HER2_vs_NBS.txt", sep="\t", header=TRUE, row.names=1)
IDX = intersect(which(updown$Adj.p.value < 0.01), which(abs(updown$M) > 1))
