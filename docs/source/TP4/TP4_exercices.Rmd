...
> **Question 1.1**:  Trouvez une brève définition des 3 sous types de cancer. Identifiez pour chaque sous-type, le ou les gènes caractéristiques. Ces informations sont définies dans les 2 articles indiqués sur le site web du TP. Y a t'il des traitement différents par sous groupe ?
...
> **Question 1.3**: Comparer l'ACP générée sur la listes des gènes spécifiques à chaques sous type par rapport à l'ACP et le clustering hierarchique obtenu sur les données non supervisées (obtenues lors du premier TP). Quels sont les sous-groupes les plus proches ?
### Faire la matrice de donnée  : sous matrice des gènes non commun (intersection 2 a 2 OK)
...
erbb = read.delim("TP4/data_TP4/ERBB.txt", header = TRUE)
