...
Les ARNm de 17 échantillons de tumeurs du sein de 3 types, HER2 positif (HER2), triple négatif (TNBC), non triple négatif (Non-TNBC), et 3 échantillons de sein normal (épithelium) ont été séquencés par Illumina HiSeq2000. Les données brutes sont disponibles sur le site du NCBI, sous sra, étude [SRP032789](ftp://ftp-trace.ncbi.nlm.nih.gov/sra/sra-instant/reads/ByStudy/sra/SRP/SRP032/SRP032789/). 
...
Afin d'obetnir la table d'expression utilisée dans ce TP, les données brutes (fastq) ont préalablement été nettoyées, alignées sur le génome de référence humain hg19 avec l'outil STAR, puis la quantification a été faite avec l'outil feature-count. Cet outil génère un tableau avec en ligne les gènes, et en colonnes les indivuds. On y retrouve l'annotation des gènes ainsi que les valeurs d'expression brutes, aussi appelées "comptages".
...
```{r classeObjet, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
class(comptage)
class(design)
dim(comptage)
dim(design)
...
```{r pieChart, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='asis'}
...
```{r comptage, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
...
```{r librarySize, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='asis'}
...
```{r normcount, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='asis'}
...
```{r boxplot, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='asis'}
par(mfrow=c(1,2))
...
```{r boxplotlog, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='asis'}
...
```{r gene0, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='asis'}
nbZero = apply(normcount,2,function(r){length(which(r == 0)) })
plot(nbZero, main="Nombre de gènes non exprimés par échantillon", xlab = "Echantillons", ylab="Nb gènes = 0", pch=16)
```
...
```{r geneAll0, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
...
```{r acp, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='asis'}
...
```{r cluster, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='asis'}
library(pheatmap)
